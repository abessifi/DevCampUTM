...
if(!function_exists('themeple_admin_get_dynamic_templates')){
    ...
    function themeple_admin_get_dynamic_templates($prepend = ''){
        ...
        if(is_array($controller->admin_pages)){
            foreach($controller->admin_pages as $page){
...
if(!function_exists('themeple_create_video_insert')){
    ...
    function themeple_create_video_insert(){            
            $video_description = '<p class="help">Progreen theme shared on W P L O C K E R . CO M - Enter the URL to the Video. <br/> A list of all supported Video Services can be found <a href="http://codex.wordpress.org/Embeds#Okay.2C_So_What_Sites_Can_I_Embed_From.3F" target="_blank">here</a>
								<br/> <br/> 
								Working examples:<br/>
