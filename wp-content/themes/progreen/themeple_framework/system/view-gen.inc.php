    ...
    class themeple_viewgen{
       ...
       var $used_for = 'admin_page';
        ...
        function themeple_viewgen($controller = false){
            if(!$controller) { $controller = $GLOBALS['controller']; }
        ...
        function page_option_header(){
            ...
            $output .= '<div class="mini_header"><h2>'.$title.' '.$this->controller->current['title'].'</h2></div>';
            $output .= $this->generate_save();
            $output .= '</div>';
            $output .= '<div class="themeple_navigation"><div class="template_description">This is your theme options panel from here you can edit, add, delete or change everything on the theme. If you need any support please contact us. Thank You</div><ul></ul></div><div class="themeple_main_section">';
        ...
        function upload_gallery($element){
			...
			$output .= '<div class="themeple_upload_gallery_container themeple_upload_gallery_container'.$postId.' themeple_delay_required">';
        ...
        function generate_element($element){
            if(isset($element['type']) && method_exists( $this, $element['type'] ) ){
				...
				if(isset($element['dynamic']))
				{
					...
					$output .= '		<div class="themeple-row-content" id="fancy_box'.$element['id'].'">';
					$output .= "		";
        ...
        function layout_section($element){
			...
			for ($i = 0; $i < $iterate; $i++)
			{
                ...
                $output   .= '<div class="themeple_set '.(isset($element['class'])?$element['class']:'').'" id="themeple_'.$element['id'].'">';
        ...
        function radioimage($element){
            ...
            if(isset($element['subtype']) && count($element['subtype']) > 0){
                ...
                foreach($element['subtype'] as $option):
                    ...
                    $output .= '        </label>';
                    $output .= '        <input type="radio" name="'.$element['id'].'" value="'.$option['value'].'" id="'.$element['id'].'"  '.$checked.'  class="'.str_replace(" ", "_", $option['name']).'"> ';
		...
		{	
			...
			else if($element['subtype'] == 'cat')
			{
                ...
                $select = 'Select staff post';
                $entries = get_posts('title_li=&orderby=name&numberposts=9999&post_type=staff');
            }else if($element['subtype'] == 'header'){
                $select = 'Select header post';
                $entries = get_posts('title_li=&orderby=name&numberposts=9999&post_type=header');
			...
			foreach ($entries as $key => $entry)
			{
				...
				if($element['subtype'] == 'page' || $element['subtype'] == 'post' || $element['subtype'] == 'portfolio' || $element['subtype'] == 'header' || $element['subtype'] == 'staff')
				{
					$id = $entry->ID;
        ...
        function change_skin($element){
            ...
            $output = '';
            foreach($predefined as $key => $pre):
                $class = '';
                if($element['std'] == $key)
                    $class = 'active_skin';
                $output .= '<a id="'.$key.'" class="themeple_btn '.$class.' change_skin" style="background-color:'.$pre['base_color'].' !important;">'.$key.'</a>';
            endforeach;
            $output .= '<span class="loading"></span>';
            return $output;
        }
        ...
        function iconset($element){
            ...
            if(isset($icons) && count($icons) > 0){
                ...
                foreach($icons as $icon):
                    ...
                    $output .= '            <div id="check-list" class="'.$cl.'"></div>';
                    $output .= '        </label>';
                    $output .= '        <input type="radio" name="'.$element['id'].'" value="'.$icon.'" id="'.$element['id'].'"  '.$checked.'  class="'.$icon.'"> ';
